...
if(!isset($_SESSION['admin']) || $_SESSION['admin'] != true){
	...
	if(mysqli_query($link, "CALL DeleteSupplier(".$_GET[delete].");")){
		echo 'Запись удалена.';
		echo "<script>location.replace('showsuppliers.php');</script>";
	}
	else {
	...
	mysqli_close($link);
		...
		echo "<script>
			...
			if(a==true){
				location.replace('deletesupplier.php?id=ok&delete=".$_GET[delete]."');
			}
			else location.replace('showsuppliers.php');
		</script>";
	}
?>