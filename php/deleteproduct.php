...
if(!isset($_SESSION['admin']) || $_SESSION['admin'] != true){
	...
	if(mysqli_query($link, "CALL DeleteProduct(".$_GET[delete].");")){
		echo 'Запись удалена.';
		echo "<script>location.replace('showproducts.php');</script>";
	}
	else {
	...
	mysqli_close($link);
		...
		echo "<script>
			...
			if(a==true){
				location.replace('deleteproduct.php?id=ok&delete=".$_GET[delete]."');
			}
			else location.replace('showproducts.php');
		</script>";
	}
?>