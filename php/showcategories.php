...
<body>
	<?php 
		...
		if($sql = mysqli_query($link, "SELECT * FROM category;")){?>
			<a href="addcategory.php" target="admin__iframe" class="admin__link admin__add">Добавить категорию</a>
			<?php
			echo "<div class='admin__table'>
					...
					for ($i=0; $i < $num_rows; $i++) { 
						while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
							echo "<div class='admin__row' id='".$row[ID_Category]."'><div class='admin__cell'>".$row[ID_Category]."</div><div class='admin__cell'>".$row[Name]."</div>";
							echo "<div class='admin__do'>
							<a href='editcategory.php?id=".$row[ID_Category]."' class='admin__link'>Изменить</a>
							<a href='deletecategory.php?delete=".$row[ID_Category]."' class='admin__link'>Удалить</a>
						</div></div>";
						}
					}
				echo "</div>";
